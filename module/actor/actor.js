...
export class GurpsActor extends Actor {
	...
	async postImport() {
		...
		if (game.settings.get(settings.SYSTEM_NAME, settings.SETTING_USE_FOUNDRY_ITEMS)) {
			...
			const validMods = userMods.filter(
				m => m.includes("@system.") || m.includes("@man:") || m.includes("@custom")
			)
			await this.update({ "system.conditions.usermods": validMods })
		}
			...
			if (!!actorComp) {
				...
				const actorCompKey =
					data.type === "equipment"
						...
						: this._findSysKeyForId(
								"uuid",
								parentItem.system[parentItem.itemSysKey].uuid,
								parentItem.actorComponentKey
							)
				...
				if (!!parentItem.system[subType] && typeof parentItem.system[subType] === "object") {
					for (const key in parentItem.system[subType]) {
						if (parentItem.system[subType].hasOwnProperty(key)) {
							...
							const commitData = await this._addChildItemElement(
								parentItem,
								childItemData,
								subType,
								newList
	...
	async addTaggedRollModifiers(chatThing, optionalArgs, attack) {
		...
		if (optionalArgs.obj) {
			...
			switch (ref) {
				...
				case "r":
					...
					refTags = refTags.concat(
						taggedSettings.allRangedRolls.split(",").map(it => it.trim().toLowerCase())
					)
					break
				case "p":
			...
			switch (ref) {
				...
				case "iq":
				case "ht":
					refTags = taggedSettings[`all${ref.toUpperCase()}Rolls`]
						.split(",")
						.map(it => it.trim().toLowerCase())
					refTags = refTags.concat(
						taggedSettings.allAttributesRolls.split(",").map(it => it.trim().toLowerCase())
					)
					break
				case "will":
				case "per":
		...
		for (const userMod of allMods) {
			...
			for (const tag of userModsTags) {
				...
				if (actorInCombat) {
					canApply =
						canApply &&
						(!taggedSettings.nonCombatOnlyTag || !modifierTags.includes(taggedSettings.nonCombatOnlyTag))
				} else {
					canApply =
						canApply &&
						(!taggedSettings.combatOnlyTag || !modifierTags.includes(taggedSettings.combatOnlyTag))
				}
				if (canApply) {
	...
	async canRoll(action, token, chatThing = "", actorComp = {}) {
		...
		const isDefense =
			action.attribute === "dodge" || action.type === "weapon-parry" || action.type === "weapon-block"
		...
		if (token && combatIsActive && isCombatant && !isSlam) {
			...
			const checkMaxActionSettings = game.settings.get(
				Settings.SYSTEM_NAME,
