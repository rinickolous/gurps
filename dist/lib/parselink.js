...
export const DAMAGE_REGEX = /^(?<accum>\+)?(?<roll>\d+)(?<D>d\d*)?(?<adds>[\–\-+]@?\w+)?(?<mult>[×x\*]\d+\.?\d*)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*?)(?<costs>\*(costs|per)? \d+ ?[\w\(\) ]+)?(?<follow>,.*)?$/i;
...
export const DERIVED_DAMAGE_REGEX = /^(?<accum>\+)?(?<att>swing|thrust|sw|thr)\s*(?<adds>[\–\-+]@?\w+)?(?<mult>[×x\*]\d+\.?\d*)? ?(?<div>\(-?[\.\d]+\))?(?<min>!)? ?(?<other>[^\*]*?)(?<costs>\*(costs|per)? \d+ ?[\w\(\) ]+])?(?<follow>,.*)?$/i;
...
const _modifier = {
    ...
    parse: (match, args) => {
        ...
        if (!desc)
            desc = args.htmldesc || ''; // htmldesc is for things like ACC columns, or to hitlocations, where the mod's description is really the column name
...
const _pdf = {
    ...
    parse: (match, args) => {
        return {
            text: "<span class='pdflink' data-pdf='" +
                match.groups.link +
                "'>" +
...
const _attribute = {
     ...
     */
    regex: /^(?<attrname>[A-Za-z][A-Za-z ]*[A-Za-z][0-9]*)(?<object>:[A-Za-z \*]+)? ?(?<mod>[+-](\d+|@margin))? ?(?<desc>[^\|]*)(?<remainder>\|.*)?$/,
    /**
     * @returns {
     *  text: string,
     *  action: {
    ...
    parse: (match, args) => {
        ...
        if (!!path) {
            let opt = match.groups.desc.trim().match(/(?<attr>[^\?]*)(\? *"(?<truetext>[^"]*)")?( *[,:] *"(?<falsetext>[^"]*)")?/) ||
...
const _skillSpell = {
     ...
     * • desc = ( text - "|" ) -- Any string that does not contain the '|' character.
     */
    regex: /^(?<type>S:|Sp:|Sk:)(?<name>"[^"]+"|'[^']+'|(?<!["'])[^|]+?(?= ?[+-](\d+|@margin))|(?<!["'])[^ ]+) ?(?<mod>[+-](\d+|@margin))? ?(?<desc>[^\|]*)(?<remainder>\|.*)?$/i,
    /**
     * @returns {
     *  text: string,
     *  action: {
    ...
    parse: (match, args) => {
        ...
        if (!modifierText && !!moddesc)
            spantext += ' ' + moddesc; // just a regular comment
        let def = null;
        if (!!remainder) {
        ...
        if (isSpell)
            prefix = blindrollText(args.blindroll) + '<b>Sp:</b>';
        if (isSkill)
            prefix = blindrollText(args.blindroll) + '<b>Sk:</b>';
        let action = {
            orig: args.str,
            type: 'skill-spell',
            isSpellOnly: isSpell,
...
const _attackDamage = {
    ...
    parse: (match, args) => {
        ...
        let type = 'attack';
        if (!!args.str.match(/^D/i))
            type = 'attackdamage';
        if (!!args.str.match(/^P/i))
            type = 'weapon-parry';
        if (!!args.str.match(/^B/i))
            type = 'weapon-block';
        let action = {
            orig: match.input,
            type: type,
        ...
        return {
            text: gspan(args.overridetxt, spantext, action, blindrollText(args.blindroll) + '<b>' + match.input[0].toUpperCase() + ':</b>', comment),
            action: action,
        };
    },
};
...
export function parselink(input, htmldesc, clrdmods = false) {
    ...
    for (const f of parseFunctions) {
        ...
        if ((matches = args.str.match(f.regex))) {
            if ((result = f.parse(matches, args)))
                return result;
        }
    }
    return { text: args.str };
...
export function parseForRollOrDamage(str, overridetxt) {
    ...
    if (!!a) {
        ...
        if (!woundingModifier) {
            ...
            if (!dice)
                return undefined; // if no damage type and no dice, not a roll, ex: [70]
            let action = {
                orig: str,
                type: 'roll',
...
function _parseOtherForTypeModiferAndLocation(other) {
    ...
    if (!!dmgTypeMatch)
        if (dmgTypeMatch[2].includes('@')) {
            ...
            extDamageType = !!dmgTypeMatch[2].trim() ? dmgTypeMatch[2].trim() : undefined; // 'ex' or 'inc' or more likely, undefined
