...
export default function () {
    ...
    Handlebars.registerHelper('pluralize', function (word, quantity) {
        if (quantity == 1)
            return word;
        if (word.slice(-1) == 's' || word.slice(-1) == 'x')
            return `${word}es`;
        return `${word}s`;
    });
    ...
    Handlebars.registerHelper('length', function (obj) {
        ...
        if (foundry.utils.getType(obj) === 'Array')
            return obj.length;
        return 0;
    });
        ...
        for (let key in context) {
            ...
            if (display) {
                ...
                if (newItem['hasContains'])
                    flatlist(item.contains, level + 1, newKey + '.contains.', data, isCollapsed);
                if (newItem['hasCollapsed'])
                    flatlist(item.collapsed, level + 1, newKey + '.collapsed.', data, true);
            }
        }
    };
    ...
    Handlebars.registerHelper('hitlocationroll', function (loc, roll, data) {
        if (!roll) {
            ...
            if (!table)
                table = HitLocations.hitlocationDictionary['humanoid'];
            roll = table[loc]?.roll;
        }
        return roll;
    });
    ...
    Handlebars.registerHelper('fractionalize', function (value, digits) {
        if (typeof value == 'number') {
            ...
            if (fraction === 1 / 3)
                return `${wholeNumberText} 1/3`.trim();
            if (fraction === 2 / 3)
                return `${wholeNumberText} 2/3`.trim();
    ...
    Handlebars.registerHelper('displayNumber', function (text, options) {
        ...
        if (!isNaN(number)) {
            if (number === 0)
                return showPlus ? '+0' : '0';
            if (number < 0)
                return text.toString().replace(hyphen, '&minus;');
            if (showPlus && text.toString()[0] !== '+')
                return `+${text}`;
    ...
    Handlebars.registerHelper('displayDecimal', function (num, options) {
        if (num != null) {
            ...
            if (parseFloat(num) < 0)
                return num.toString().replace('-', '&minus;');
    ...
    Handlebars.registerHelper('isFoundryGlobalItem', function (obj, doc) {
        ...
        return (game.settings.get(settings.SYSTEM_NAME, settings.SETTING_SHOW_FOUNDRY_GLOBAL_ITEMS) && !!item?.system.globalid);
    });
    ...
    Handlebars.registerHelper('parentItemTooltip', function (obj, doc) {
        ...
        return !!parentItem
            ? new Handlebars.SafeString(game.i18n.format('GURPS.parentItemTooltip', { name: parentItem.name, ['type']: parentItem.type }))
            : '';
    });
    Handlebars.registerHelper('globalItemTooltip', function (obj, doc) {
        ...
        if (!!item?.system.globalid)
            item = game.items.find(it => it.id === item.system.globalid.split('.').pop());
        return !!item
            ? new Handlebars.SafeString(game.i18n.format('GURPS.parentItemTooltip', { name: item.name, ['type']: `game ${item.type}` }))
    ...
    Handlebars.registerHelper('displayItemHover', function (obj, doc) {
        if (!game.settings.get(settings.SYSTEM_NAME, settings.SETTING_SHOW_FOUNDRY_CREATED))
            return;
        if (!!obj.img && obj.img !== 'icons/svg/item-bag.svg')
            return obj.img;
        const actor = doc.data?.root?.document;
    ...
    Handlebars.registerHelper('collapsible-content', function (id, data, group, options) {
        ...
        let content = data
            .slice(1)
            .map((/** @type {string} */ it) => it.startsWith('*')
...
`;
    ...
    Handlebars.registerHelper('damageTerm', function (calc, options) {
        let armorDivisor = calc.useArmorDivisor && calc.armorDivisor //
            ? calc.armorDivisor == -1
                ? '(∞)'
                : `(${calc.armorDivisor})` //
            : '';
        let damageType = calc.damageType !== 'dmg' && calc.damageType !== 'injury' && calc.damageType !== 'none' ? calc.damageType : '';
