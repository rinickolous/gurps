...
export class GurpsDie extends foundry.dice.terms.Die {
    ...
    asDiceTerm() {
        if (this instanceof foundry.dice.terms.DiceTerm)
            return this;
    ...
    roll({ minimize = false, maximize = false } = {}) {
        ...
        if (CONFIG.debug.dice)
            console.log(`Loaded Die [${this.baseExpression()}] -- values: ${this._loaded}`);
    ...
    sum() {
        ...
        if (taggedSettings.autoAdd && this.currentSum !== oldSum) {
            ...
            if (!!target && !isNaN(target)) {
                ...
                if (this.maxTotal)
                    total = Math.min(total, this.maxTotal);
    ...
    _add(list, mod, reason = '', replace = false, tagged = false) {
        ...
        if (i > -1) {
            if (replace)
                ...
                oldmod = list[i]; // Must modify list (cannot use filter())
        }
...
export class ModifierBucket extends Application {
    ...
    addModifier(mod, reason, list, tagged = false) {
        if (!!list)
            this.modifierStack._add(list, mod.toString(), reason, false, tagged);
        else
            this.modifierStack.add(mod.toString(), reason, false, tagged);
        this.refresh();
    }
    currentSum() {
    ...
    sendToPlayers(action, usernames) {
        ...
        if (_users) {
            ...
            if (usernames.length > 0)
                players = players.filter(u => u.name && usernames.includes(u.name));
            if (!!players)
                this._sendBucket(players);
    ...
    sendBucketToPlayer(id) {
        ...
        if (!id) {
            ...
            if (users.length > 0)
                ...
                ui.notifications?.warn("No player with ID '" + id + "'");
        }
    }
    resetActorModifiers() {
    ...
    activateListeners(html) {
        ...
        if (this.isTooltip)
            e.on('mouseenter', ev => this._onenter(ev));
    ...
    async showOthers() {
        ...
        for (let user of users || []) {
            ...
            if (!!stack && !!stack.modifierList)
                content += this.chatString(stack, user.name + ', ');
            else
                content += user.name + ', No modifiers';
        }
        let chatData = {};
