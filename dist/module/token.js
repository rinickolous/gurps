...
export default class GurpsToken extends Token {
    ...
    async setManeuver(maneuverName) {
        ...
        if (isCombatActive) {
            ...
            if (maneuvers && maneuvers.length === 1) {
                ...
                if (maneuvers[0].getFlag('gurps', 'name') !== maneuverName)
    ...
    _refreshIcon() {
        ...
        if (aspect == 1) {
            if (this.w > this.h)
                aspect = 0.9; // force scaling by height when width is greater than height
        }
        if (aspect >= 1) {
