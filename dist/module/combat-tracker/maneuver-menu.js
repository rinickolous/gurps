...
export const addManeuverMenu = async (html, combatant, token) => {
    ...
    if (!actorManeuverName || actorManeuverName === 'undefined')
        actorManeuverName = 'do_nothing';
    ...
    const maneuverTooltip = $(await renderTemplate('systems/gurps/templates/maneuver-button-tooltip.hbs', {
        label,
        maxMove,
        allIcons,
    }));
...
export const addManeuverListeners = () => {
    $(document)
        ...
        const token = canvas.tokens.get(combatant.token.id);
        const currentManeuver = foundry.utils.getProperty(token.actor, 'system.conditions.maneuver');
        if (currentManeuver === maneuverName)
            return;
        await token.actor.update({
