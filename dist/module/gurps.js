...
import { arrayToObject, atou, d6ify, makeRegexPatternFrom, objectToArray, quotedAttackName, recurselist, sanitize, utoa, wait, zeroFill, } from '../lib/utilities.js';
import RegisterChatProcessors from './chat/chat-processors.js';
import { GurpsActorCombatSheet, GurpsActorEditorSheet, GurpsActorNpcSheet, GurpsActorSheet, GurpsActorSheetReduced, GurpsActorSimplifiedSheet, GurpsActorTabSheet, GurpsInventorySheet, } from './actor/actor-sheet.js';
...
`;
    ...
    GURPS.setLastTargetedRoll = function (chatdata, actorid, tokenid, updateOtherClients = false) {
        ...
        if (!!actorid)
            GURPS.lastTargetedRolls[actorid] = tmp;
        if (!!tokenid)
            GURPS.lastTargetedRolls[tokenid] = tmp;
    ...
    function objToString(obj, ndeep = 1) {
        ...
        if (ndeep > 10)
            return '(stopping due to depth): ' + obj.toString();
        switch (typeof obj) {
            case 'string':
                return '"' + obj + '"';
            case 'function':
    ...
    async function executeOTF(inputstring, priv = false, event = null, actor = null) {
        ...
        for (let string of strings) {
            ...
            if (!!action.action) {
                if (!event)
                    event = { shiftKey: priv, ctrlKey: false, data: {} };
    ...
    function processSkillSpell({ action, actor }) {
        ...
        // @ts-ignore
            ...
            skill = GURPS.findSkillSpell(actor?.system, action.name, !!action.isSkillOnly, !!action.isSpellOnly);
        if (!skill) {
            return 0;
        }
    ...
    const actionFuncs = {
        ...
        async chat({ action, actor, event }) {
            ...
            if (actor)
                GURPS.SetLastActor(actor); // try to ensure the correct last actor.
        ...
        async damage({ action, event, actor, targets }) {
            ...
            if (actor)
                canRoll = await actor.canRoll(action, token);
            if (!canRoll.canRoll) {
            ...
            if (!!action.costs)
                GURPS.ModifierBucket.addModifier(0, action.costs);
            if (!!action.mod)
            ...
            return await Damage.rollDamage(canRoll, token, actor, displayFormula, action.formula, action, event, null, targets);
        },
        /**
         * @param {Object} data
         *
        ...
        async attack({ action, actor, event }) {
            ...
            let p = 'A:';
            if (!!action.isMelee && !action.isRanged)
                p = 'M:';
            if (!action.isMelee && !!action.isRanged)
                p = 'R:';
            ...
            let targetmods = [];
            if (opt.obj.checkotf && !(await GURPS.executeOTF(opt.obj.checkotf, false, event, actor)))
                return false;
            ...
            if (action.overridetxt)
                opt.text += "<span style='font-size:85%'>" + action.overridetxt + '</span>';
            return await doRoll({
                actor,
                targetmods,
                thing,
        ...
        ['test-exists']({ action, actor }) {
            switch (action.prefix) {
                case 'A':
                    if (!!findAdDisad(actor, action.name))
                        return true;
                    if (!!findAttack(actor, action.name, true, true))
                        return true;
                    return false;
                case 'AD':
                    if (!!findAdDisad(actor, action.name))
                ...
                case 'AT':
                    ...
                    return false;
                case 'M':
                    if (!!findAttack(actor, action.name, true, false))
                        return true;
                    return false;
                case 'R':
                    if (!!findAttack(actor, action.name, false, true))
                        return true;
                    return false;
                case 'S':
                    if (!!findSkillSpell(actor, action.name, false, false))
                        return true;
                    return false;
                case 'SK':
                    if (!!findSkillSpell(actor, action.name, true, false))
                        return true;
                    return false;
                case 'SP':
                    if (!!findSkillSpell(actor, action.name, false, true))
                        return true;
                    return false;
            }
            return false;
    ...
    async function performAction(action, actor, event = null, targets = []) {
        if (!action || !(action.type in actionFuncs))
            return false;
        if (action.sourceId)
            actor = game.actors.get(action.sourceId);
    ...
    function findSkillSpell(actor, sname, isSkillOnly = false, isSpellOnly = false) {
        ...
        if (!actor)
            return t;
        if (actor instanceof GurpsActor)
            actor = actor.system;
    ...
    async function handleRoll(event, actor, options) {
        ...
        if ('damage' in element.dataset) {
            ...
            if (opt.text === text)
                ...
                opt.text = "<span style='font-size:85%'>(" + opt.text + ')</span>';
            let k = $(element).closest('[data-key]').attr('data-key');
            if (!k)
                k = element.dataset.key;
            if (!!k) {
                if (actor)
                    opt.obj = foundry.utils.getProperty(actor, k); // During the roll, we may want to extract something from the object
                if (opt.obj.checkotf && !(await GURPS.executeOTF(opt.obj.checkotf, false, event, actor)))
                    return;
                if (opt.obj.duringotf)
                    await GURPS.executeOTF(opt.obj.duringotf, false, event, actor);
            ...
            if (!!t) {
                ...
                // Can't roll against a non-integer
                    ...
                    let m = a.join(' ');
                    if (!!m)
                        GURPS.ModifierBucket.addModifier(0, m);
                }
            }
        }
    ...
    function put(obj, value, index = -1) {
        if (index == -1) {
            index = 0;
            while (obj.hasOwnProperty(zeroFill(index)))
                index++;
        }
        ...
        for (let key in eqts) {
            ...
            if (!!src && game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_REMOVE_UNEQUIPPED)) {
                ...
                recurselist(src.equipment.carried, e => {
                    if (eqt.name.startsWith(e.name) && !e.equipped)
                        display = false;
                });
            }
            if (display) {
    ...
    GURPS.setInitiativeFormula = function (/** @type {boolean} */ broadcast) {
        ...
        if (!formula) {
            ...
            if (game.user.isGM)
                game.settings.set(Settings.SYSTEM_NAME, Settings.SETTING_INITIATIVE_FORMULA, formula);
        }
...
<div style='font-size:70%'>
  ...
  <div style='text-align: center;'>
    ...
    <div><a href="https://ko-fi.com/crnormand"><img height="24" src="systems/gurps/icons/SupportMe_stroke@2x.webp"></a></div>
        ...
        resourceTrackers.forEach(it => (GURPS.DamageTables.woundModifiers[it.alias] = {
            multiplier: 1,
            label: it.name,
            resource: true,
        }));
        Hooks.on('hotbarDrop', async (_bar, data, slot) => {
            if (!data.otf && !data.bucket)
                return;
        ...
        game.socket.on('system.gurps', async (resp) => {
            ...
            if (resp.type == 'dragEquipment1') {
                if (resp.destuserid != game.user.id)
                    return;
            ...
            if (resp.type == 'dragEquipment2') {
                if (resp.srcuserid != game.user.id)
                    return;
                let srcActor = game.actors.get(resp.srcactorid);
        ...
        Hooks.on('controlToken', (...args) => {
            if (GURPS.IgnoreTokenSelect)
                return;
            if (args.length > 1) {
                let a = args[0]?.actor;
                if (!!a) {
                    if (args[1])
                        GURPS.SetLastActor(a, args[0].document);
                    else
                        GURPS.ClearLastActor(a);
                }
            }
        });
