...
export async function rollDamage(canRoll, token, actor, displayFormula, actionFormula, action, event, overrideText, targets) {
    ...
    if (showRollDialog && !canRoll.isSlam) {
        ...
        const damageTypeLabel = game.i18n.localize(`GURPS.damageTypes.${GURPS.DamageTables.woundModifiers[damageType]?.label}`, damageType);
                ...
                buttons: [
                    {
                        ...
                        callback: async (event, button, dialog) => {
                            await DamageChat.create(actor || game.user, actionFormula, action.damagetype, event, overrideText, targets, action.extdamagetype, action.hitlocation);
                            if (action.next) {
