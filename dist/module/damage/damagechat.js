...
export default class DamageChat {
    ...
    static async _renderDamageChat(app, html, _msg) {
        if (!html.find('.damage-chat-message').length)
            return; // this is not a damage chat message
        ...
        if (!!transfer.userTarget && transfer.userTarget != null) {
            if (game.user.isGM) {
                ...
                button.on('click', ev => {
                    ...
                    if (!!actor)
                        actor.handleDamageDrop(transfer.payload);
                    else
     ...
     */
    static async create(actor, diceText, damageType, event, overrideDiceText, tokenNames, extdamagetype = null, hitlocation = null) {
    ...
    _getDiceData(originalDiceText, damageType, targetmods, overrideDiceText, extdamagetype) {
        ...
        if (!!result.groups?.D) {
            rolled = true;
            if (result.groups.D === 'd')
                formula = d6ify(diceText, '[Damage]'); // GURPS dice (assume 6)
        }
        ...
        if (isNiceDiceEnabled()) {
            ...
            rolls.forEach(roll => {
                roll.dice.forEach(die => {
                    ...
                    die.results.forEach(s => dice.push({
                        result: s.result,
    ...
    static async _calculateAndSelectTargets(canvas, dropData) {
        ...
        if (selectedTokens.length > 0) {
            for (let target of selectedTokens) {
                ...
                if (target.actor)
                    await target.actor.handleDamageDrop(dropData.payload);
            }
            return false;
        }
        return true;
        function isValidUser(user, attackerActor) {
            ...
            if (!userOwnsAttacker)
                ui.notifications?.warn(game.i18n.localize('GURPS.noUserForDamageWarning'));
