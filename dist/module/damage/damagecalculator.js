...
export class CompositeDamageCalculator {
    ...
    constructor(defender, damageData) {
        ...
        if (hlIndex >= 0)
            this._hitLocation = hitlocations[hlIndex].where;
        else
    ...
    isUnliving(values, found) {
        ...
        if (!found) {
            ...
            found = values.find(value => {
                let found = !!(['Injury Tolerance (Unliving)', 'Unliving'].includes(value.name) ||
    ...
    isHomogenous(values, found) {
        if (!found) {
            let self = this;
            found = values.find(value => {
                let found = !!(['Injury Tolerance (Homogenous)', 'Homogenous'].includes(value.name) ||
    ...
    isDiffuse(values, found) {
        if (!found) {
            let self = this;
            found = values.find(value => {
                let found = !!(['Injury Tolerance (Diffuse)', 'Diffuse'].includes(value.name) ||
    ...
    get DR() {
        if (this._userEnteredDR !== null)
            return this._userEnteredDR;
        if (this._hitLocation === 'Random')
            return 0;
        ...
        if (this._hitLocation === 'Large-Area')
            return HitLocationEntry.getLargeAreaDR(entries);
    ...
    async addEffectsContext() {
        ...
        if (!tokenId)
            tokenId = canvas.tokens.placeables.find(it => it.actor === this._defender).id;
        ...
        if (this._isInjuryTolerance) {
            switch (this._injuryToleranceType) {
                case UNLIVING:
                    ...
                    if (['Eye', 'Skull', 'Vitals'].includes(this._hitLocation))
                        break;
    ...
    get isCrippleableLocation() {
        return ([hitlocation.EXTREMITY, hitlocation.LIMB, hitlocation.CHEST, hitlocation.GROIN].includes(this.hitLocationRole) ||
    ...
    get cripplingThreshold() {
        if (this.hitLocationRole === hitlocation.LIMB)
            return this.HP.max / 2;
        if (this.hitLocationRole === hitlocation.EXTREMITY)
            return this.HP.max / 3;
        if (this._useBodyHits) {
            if (this.hitLocationRole === hitlocation.CHEST)
                return this.HP.max * 2;
            if (this.hitLocationRole === hitlocation.GROIN)
                return this.HP.max * 2;
        }
        else {
            if (this.hitLocationRole === hitlocation.CHEST)
        ...
        if (this.hitLocation === 'Eye')
            return this.HP.max / 10;
        return Infinity;
    }
    get locationMaxHP() {
    ...
    get resource() {
        ...
        if (!!tracker)
            return [tracker, `system.additionalresources.tracker.${zeroFill(index, 4)}`];
        // }
        if (this._applyTo === 'FP')
            return [this._defender.system.FP, 'system.FP'];
...
class DamageCalculator {
    ...
    get calculatedBluntTrauma() {
        if (this.effectiveDamage === 0 || this.penetratingDamage > 0)
            return 0;
        if (!bluntTraumaTypes.includes(this._parent.damageType))
            return 0;
    ...
    get pointsToApply() {
        ...
        if (this._parent.useLocationModifiers) {
            ...
            else if ([hitlocation.GROIN, hitlocation.CHEST].includes(this._parent.hitLocationRole) &&
    ...
    get penaltyToHTRollForStunning() {
        ...
        if (this.isMajorWound && ['Face', 'Vitals', 'Groin'].includes(this._parent.hitLocation))
            return 5;
        ...
        if (this.isMajorWound && ['Eye', 'Skull', 'Brain'].includes(this._parent.hitLocation))
            return 10;
        return 0;
    }
    ...
    get isKnockbackEligible() {
        ...
        if (this._parent._damageModifier === 'nkb')
            return false;
    ...
    get effects() {
        ...
        if (this.isKnockbackEligible) {
            ...
            if (this._parent._damageModifier === 'dkb')
                knockback *= 2;
            if (knockback > 0) {
    ...
    get isBluntTraumaInjury() {
        return (this.injury === 0 && this._parent.isFlexibleArmor && this._parent.useBluntTrauma && this.effectiveBluntTrauma > 0);
    }
    get maxInjuryForDiffuse() {
        return this._maxInjuryForDiffuse;
    }
