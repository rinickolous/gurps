...
import { displayMod, generateUniqueId, isNiceDiceEnabled, locateToken, objectToArray, parseFloatFrom, parseIntFrom, } from '../../lib/utilities.js';
...
export default class ApplyDamageDialog extends Application {
    ...
    activateListeners(html) {
        ...
        html.find('#pagination-left').on('click', ev => {
            if (this._calculator.viewId === 'all')
                return;
            if (this._calculator.viewId === 0)
                this._calculator.viewId = 'all';
            else
                this._calculator.viewId = this._calculator.viewId - 1;
            this.updateUI();
        });
        html.find('#pagination-right').on('click', ev => {
            if (this._calculator.viewId === 'all')
                this._calculator.viewId = 0;
            else {
                let index = this._calculator.viewId + 1;
                if (index >= this._calculator.length)
                    return;
        ...
        html
            ...
            .on('change', ev => this._updateModelFromInputText($(ev.currentTarget), 'userEnteredWoundModifier', parseFloatFrom));
        // When 'Additional Mods' text changes, save the (numeric) value in this object and
        // update the result-addmodifier, if necessary.
        html
            .find('#addmodifier')
            .on('change', ev => this._updateModelFromInputText($(ev.currentTarget), 'additionalWoundModifier', parseFloatFrom));
        html
        ...
        html
            .find('#add-shock-next button, #add-major button, #add-crippling button, #add-knockback button, #add-vitals button')
     ...
     */
    _updateModelFromSelect(select, property, converter = value => {
        return value;
    }) {
    ...
    async _handleTestSaveEffectButtonClick(ev) {
        ...
        switch (effect.type) {
            ...
            case 'majorwound':
            case 'crippling':
                const htCheck = (effect?.modifier ?? 0) === 0
                    ? 'HT'
                    : effect.modifier < 0
                        ? `HT+${-effect.modifier}`
                        : `HT-${effect.modifier}`;
            ...
            case 'knockback':
                ...
                const localeAcrobaticsCheck = effect?.modifier && effect.modifier === 0
                    ? localeAcrobaticsName
                    : `${localeAcrobaticsName} -${effect.modifier}`;
                const localeJudoName = game.i18n.localize('GURPS.skillJudo');
                const localeJudoCheck = effect?.modifier && effect.modifier === 0 ? localeJudoName : `${localeJudoName} -${effect.modifier}`;
                otf = `/r [!${dxCheck} | Sk:${localeAcrobaticsCheck} | Sk:${localeJudoCheck}]`;
                break;
        }
        if (!!otf)
            await this.actor.runOTF(otf);
    }
    async _handleEffectAddEffectButtonClick(ev) {
        ...
        if (!tokenId)
            tokenId = canvas.tokens.placeables.find(it => it.actor === this.actor).id;
        ...
        const toggleEffect = async (effect, span, starts = '', label = '') => {
            ...
            const effectExists = token.actor.effects.filter(e => !!starts ? e._source.statuses[0].startsWith(starts) : e._source.statuses[0] === effect);
    ...
    async _handleEffectButtonClick(ev) {
        ...
        if (object.type === 'majorwound') {
            let htCheck = object.modifier === 0 ? 'HT' : object.modifier < 0 ? `HT+${-object.modifier}` : `HT-${object.modifier}`;
    ...
    async resolveInjury(keepOpen, injury, publicly, results = null) {
        ...
        if (!resource || !path) {
            ui.notifications.warn(`Actor ${this.actor.name} does not have a resource named "${this._calculator.damageType}"!!`);
            return;
        }
