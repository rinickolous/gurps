...
export const AddImportEquipmentButton = async function (html) {
    let button = $('<button class="import-items"><i class="fas fa-file-import"></i>' +
...
export class ItemImporter {
    ...
    _getItemCost(i) {
        ...
        let value;
        if (!!i.calc?.extended_value)
            value = parseFloat(i.calc.extended_value);
        if (!value)
    ...
    _getItemWeight(i) {
        ...
        let weight;
        if (!!i.calc?.extended_weight)
            weight = parseFloat(i.calc.extended_weight);
        if (!weight)
