...
import Maneuvers, { MOVE_FULL, MOVE_HALF, MOVE_NONE, MOVE_ONE, MOVE_ONETHIRD, MOVE_STEP, MOVE_TWO_STEPS, MOVE_TWOTHIRDS, } from './actor/maneuver.js';
/**
 * # Actor Actions Class
 *
...
export class TokenActions {
    ...
    async addModifiers() {
        ...
        if (this.toHitBonus !== 0) {
            ...
            if (this.currentManeuver === 'move_and_attack') {
                addModifier(`${signal}${Math.abs(this.toHitBonus)} ${signalLabel} *Max:9 #melee #maneuver @man:${this.currentManeuver}`);
                addModifier(`${signal}${Math.abs(this.toHitBonus / 2)} ${signalLabel} #ranged #maneuver @man:${this.currentManeuver}`);
            }
            else {
        ...
        if (this.defenseBonus !== 0) {
            ...
            addModifier(`${signal}${Math.abs(this.defenseBonus)} ${signalLabel} #parry #block #dodge #maneuver @man:${this.currentManeuver}`);
        }
        if (this.evaluateTurns > 0) {
            addModifier(`+${this.evaluateTurns} ${game.i18n.localize('GURPS.toHitBonus')} #hit #maneuver @man:evaluate`);
        }
        Object.keys(this.currentParry).map(k => {
            ...
            if (parry.currentPenalty !== 0) {
                ...
                addModifier(`${signal}${Math.abs(parry.currentPenalty)} ${signalLabel} ${parry.name} #parry #maneuver #${parry.mode} @${parry.key}`);
            }
        });
    ...
    async consumeAction(action, chatThing, actionObj, usingRapidStrike = false) {
        if (!this.actor.canConsumeAction(action, chatThing, actionObj))
            return;
        ...
        switch (actionType) {
            case 'b':
                ...
                break;
            case 'p':
                const settingsAddParryMods = game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_ADD_CUMULATIVE_PARRY_PENALTIES);
    ...
    async addToNextTurn(effectNames) {
        ...
        if (!this.lastManeuvers[index])
            this.lastManeuvers[index] = this._getNewLastManeuvers();
    ...
    async removeFromNextTurn(effectNames) {
        if (!this.lastManeuvers[this.currentTurn])
            this.lastManeuvers[this.currentTurn] = this._getNewLastManeuvers();
