...
class HelpChatProcessor extends ChatProcessor {
    ...
    async process(_line, _msgs) {
        let l = _line.split(' ');
        if (l.length > 1)
            return this.registry.handle('?' + l[1].trim());
...
class ChatProcessorRegistry {
    ...
    willTryToHandle(message) {
        ...
        for (const line of lines)
            for (const p of this._processors) {
                if (p.matches(line) || (line[0] === '!' ? p.matches(line.substring(1)) : p.matches(line)))
                    return true;
            }
        return false;
    }
    ...
    async processLine(line) {
        ...
        if (!handled) {
            if (line.trim().startsWith('/')) {
                ...
                this.pub(line); // If not handled, must just be public text
        }
    ...
    async handle(line) {
        ...
        if (!!processor) {
            if (processor.isGMOnly() && !game.user?.isGM)
                ui.notifications?.warn(game.i18n.localize('GURPS.chatYouMustBeGM'));
            else {
                try {
    ...
    priv(txt, force = false) {
        if (this.msgs.quiet && !force)
            return;
        if (this.msgs.pub.length > 0)
            this.send();
...
export default function addChatHooks() {
    Hooks.once('init', async function () {
        ...
        Hooks.on('chatMessage', (_log, message, chatmsgData) => {
            // @ts-ignore
            if (!!chatmsgData.alreadyProcessed)
                return true; // The chat message has already been parsed for GURPS commands show it should just be displayed
            if (GURPS.ChatCommandsInProcess.includes(message)) {
                GURPS.ChatCommandsInProcess = GURPS.ChatCommandsInProcess.filter((/** @type {string} */ item) => item !== message);
        ...
        Hooks.on('preCreateChatMessage', (/** @type {ChatMessage} */ chatMessage, /** @type {any} */ _options, /** @type {any} */ _userId) => {
        ...
        Hooks.on('diceSoNiceRollComplete', async (/** @type {any} */ _app, /** @type {any} */ _html, /** @type {any} */ _msg) => {
            let otf = GURPS.PendingOTFs.pop();
            while (otf) {
                let action = parselink(otf);
                if (!!action.action)
                    await GURPS.performAction(action.action, GURPS.LastActor || game.user);
