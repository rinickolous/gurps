...
export class GurpsItem extends Item {
    ...
    async toogleEquip(equipped) {
        if (this.type !== 'equipment' || !this.system.carried || this.system.equipped === equipped)
            return;
    ...
    get actorComponentKey() {
        ...
        if (!sysKey)
            throw new Error(`No actor system key found for ${this.type}`);
        return sysKey;
    }
    /**
    ...
    get itemSysKey() {
        ...
        const sysKey = keys[this.type];
        if (!sysKey)
            throw new Error(`No item system key found for ${this.type}`);
        return sysKey;
    }
    /**
