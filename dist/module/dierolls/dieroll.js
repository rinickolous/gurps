...
export const addBucketToDamage = (formula, addDamageType = true) => {
    ...
    const newDice = `${dice}d${plus}${addText}${multiplierText}${minDamageText}${armorDivisorText}${damageTypeText}${costFormulaText}`.trim();
    console.debug(`addBucketToDamage: ${formula} => ${newDice}`);
    return newDice;
};
export async function doRoll({ actor, formula = '3d6', targetmods = [], prefix = '', thing = '', chatthing = '', origtarget = -1, optionalArgs = {}, fromUser = game.user, action = null, }) {
    if (origtarget == 0 || isNaN(origtarget))
        return; // Target == 0, so no roll.  Target == -1 for non-targetted rolls (roll, damage)
    ...
    if (actor instanceof Actor && action) {
        const actorTokens = canvas.tokens?.placeables.filter(t => {
            if (t.actor)
    ...
    const isAttributeOrCheckRoll = Object.keys(GURPS.PARSELINK_MAPPINGS).includes(chatthing.split('@').pop().slice(0, -1).toUpperCase());
    ...
    if (showRollDialog && actor instanceof Actor) {
        ...
        if (origtarget > 0)
            targetRoll += `-${origtarget}`;
        let template = 'confirmation-roll.hbs';
        // If we receive a single formula
        // like `/1d6+2` we need
        // to process it like a single roll
        let damageRoll, damageType, damageTypeLabel, damageTypeIcon, damageTypeColor, simpleFormula, originalFormula, otfDamageText, usingDiceAdd;
        if (!action && !chatthing && targetData.name === formula) {
            ...
            if (['thrust', 'swing', 'sw', 'thr'].includes(damageOTFType.toLowerCase())) {
                ...
                damageType = 'dmg';
                damageTypeLabel = game.i18n.localize(`GURPS.damageType${GURPS.DamageTables.woundModifiers[damageType]?.label}`, damageType);
        ...
        const settingsAllowAfterMaxActions = game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_ALLOW_AFTER_MAX_ACTIONS);
        const settingsUseMaxActions = game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_USE_MAX_ACTIONS);
        const dontShowMaxActions = settingsUseMaxActions === 'Disable' || (!result.isCombatant && settingsUseMaxActions === 'AllCombatant');
...
async function _doRoll({ actor, formula = '3d6', targetmods = [], prefix = '', thing = '', chatthing = '', origtarget = -1, optionalArgs = {}, fromUser = game.user, }) {
    if (origtarget == 0 || isNaN(origtarget))
        return; // Target == 0, so no roll.  Target == -1 for non-targetted rolls (roll, damage)
    ...
    if (isTargeted) {
        ...
        if (!!maxtarget && finaltarget > maxtarget)
        ...
        if (game.dice3d && !game.dice3d.messageHookDisabled) {
            ...
            if (failure && optionalArgs.obj?.failotf)
                GURPS.PendingOTFs.unshift(optionalArgs.obj.failotf);
            if (!failure && optionalArgs.obj?.passotf)
                GURPS.PendingOTFs.unshift(optionalArgs.obj.passotf);
        }
        else {
            if (failure && optionalArgs.obj?.failotf)
                GURPS.executeOTF(optionalArgs.obj.failotf, optionalArgs.event);
            if (!failure && optionalArgs.obj?.passotf)
                GURPS.executeOTF(optionalArgs.obj.passotf, optionalArgs.event);
        }
        let r = {};
        r['rtotal'] = rtotal;
    ...
    if (isTargeted && !!optionalArgs.action) {
        ...
        if (!failure && !!optionalArgs.action.truetext)
            messageData.content = optionalArgs.action.truetext;
        if (failure && !!optionalArgs.action.falsetext)
            messageData.content = optionalArgs.action.falsetext;
