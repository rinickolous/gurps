...
class GurpsActorSheetV2 extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2) {
    /* -------------------------------------------- */
    /*  Sheet Editing                               */
    /* -------------------------------------------- */
    /**
    ...
    async _prepareContext(options) {
        ...
        return {
            ...
            portraitHoverText: game.i18n?.localize(`GURPS.Sheet.Common.PortraitHoverText.${this.isEditing ? 'Edit' : 'View'}`),
        };
    }
    /**
    ...
    /* -------------------------------------------- */
    static async #onViewImage(event) {
        event.preventDefault();
        if (!this.actor.img || this.actor.img === '')
            return;
    ...
    static async #onToggleMode(event) {
        ...
        if (this._mode === MODES.PLAY)
            this._mode = MODES.EDIT;
        else
            this._mode = MODES.PLAY;
        await this.submit();
        this.render();
    }
}
