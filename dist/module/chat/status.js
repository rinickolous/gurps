...
export default class StatusChatProcessor extends ChatProcessor {
    ...
    async process(_) {
        if (!this.match)
            throw new Error('match does not exist! Did you call ChatProcessor.matches()?');
        let commandText = this.match.groups?.command;
        let theCommand = Command[commandText];
        if (theCommand === Command.list)
            return this.priv(this.list());
        ...
        if (theCommand === Command.clear)
            return await this.clear(tokens);
        ...
        if (!effect) {
            ...
            else if (!effectText.match(new RegExp(makeRegexPatternFrom(GURPS.StatusEffectStanding), 'i')) &&
        ...
        if (theCommand == Command.toggle)
            return await this.toggle(tokens, effect);
        else if (theCommand == Command.set)
            return await this.set(tokens, effect);
        else if (theCommand == Command.unset)
            return await this.unset(tokens, effect);
        else
            ui.notifications.warn(`Unexpected error: command: ${theCommand} (${this.match})`);
    }
    /**
    ...
    findEffect(statusText) {
        ...
        Object.values(CONFIG.statusEffects).forEach(s => {
            if (game.i18n.localize(s.name).match(pattern))
                effect = s; // match on name or id (shock1, shock2, etc.)
            if (s.id.match(pattern))
                effect = s;
        });
        return effect;
    }
    ...
    async toggleTokenEffect(token, effect, actionText) {
        if (!!effect) {
            ...
            this.prnt(`${game.i18n.localize(actionText)} <i>${effect.id}:'${game.i18n.localize(effect.label)}'</i> ${game.i18n.localize('GURPS.for')} ${actor.displayname}`);
        }
    }
    /**
     * @param {Token[]} tokens
    ...
    getStatusEffect(actorEffect) {
        for (const status of Object.values(CONFIG.statusEffects))
            if (actorEffect.statuses.has(status.id))
                return status;
