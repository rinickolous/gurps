...
export class GurpsItemSheet extends ItemSheet {
    ...
    getData() {
        ...
        if (!!this.item.system.eqt)
            sheetData.data.eqt.f_count = this.item.system.eqt.count; // hack for Furnace module
        if (this.item.ski && this.item.ski.consumeAction === undefined)
            sheetData.data.ski.consumeAction = false;
        if (this.item.spl && this.item.spl.consumeAction === undefined)
            sheetData.data.spl.consumeAction = true;
    ...
    async _onDrop(event) {
        ...
        if (!['melee', 'ranged', 'skills', 'spells', 'ads', 'equipment'].includes(dragData.type))
            return;
    ...
    async close() {
        ...
        if (!!actor) {
            const actorCompKey = this.item.type === 'equipment'
            ...
            if (!(await actor._sanityCheckItemSettings(actorComp)))
                return;
    ...
    get isContainer() {
        return (Object.keys(this.item.system.melee || {}).length > 0 ||
