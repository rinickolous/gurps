...
import { Advantage, Encumbrance, Equipment, Language, Melee, Modifier, Note, Ranged, Reaction, Skill, Spell, } from './actor-components.js';
...
export class ActorImporter {
    ...
    async _openImportDialog() {
        if (game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_USE_BROWSER_IMPORTER))
            ...
            await this._openLocallyHostedImportDialog();
    }
    async _openNonLocallyHostedImportDialog() {
        try {
            ...
            if (res)
                SmartImporter.setFileForActor(this.actor, file);
        }
        catch (e) {
    ...
    async _openLocallyHostedImportDialog() {
        new foundry.applications.api.DialogV2({
            ...
            content: await renderTemplate('systems/gurps/templates/import-gcs-v1-data.hbs', SmartImporter.getTemplateOptions(this.actor)),
            buttons: [
                {
                    action: 'import',
                    label: 'GURPS.import',
    ...
    async importActorFromExternalProgram(source, importName, importPath, suppressMessage = false) {
        if (importName.endsWith('.gcs'))
            return this.importActorFromGCS(source, importName, importPath, suppressMessage);
    ...
    async importActorFromGCS(json, importname, importpath, suppressMessage = false) {
        ...
        try {
            ...
            msg.push(game.i18n.format('GURPS.importGenericError', {
                name: nm,
                error: err.name,
                message: err.message,
            }));
        ...
        try {
            ...
            if (!suppressMessage)
                ui.notifications?.info(game.i18n.format('GURPS.importSuccessful', { name: nm }));
            ...
            if (err.message == 'Maximum depth exceeded')
                msg.push(game.i18n.localize('GURPS.importTooManyContainers'));
    ...
    async importActorFromGCA(source, importName, importPath, suppressMessage) {
        ...
        if (!r) {
            if (importName.endsWith('.gca5'))
                msg.push(game.i18n.localize('GURPS.importCannotImportGCADirectly'));
            if (importName.endsWith('.gca4'))
                msg.push(game.i18n.localize('GURPS.importCannotImportGCADirectly'));
            else if (!xml.startsWith('<?xml'))
                msg.push(game.i18n.localize('GURPS.importNoXMLDetected'));
            exit = true;
        }
        else {
        ...
        if (msg.length > 0) {
            ...
            if (exit)
                return false; // Some errors cannot be forgiven ;-)
        }
        ...
        if (this.isUsingFoundryItems()) {
            await aRecurselist(this.actor.system.ads, async (t) => {
                if (!!t.itemid) {
                    const i = this.actor.items.get(t.itemid);
                    if (!!i?.system.globalid) {
                        if (!(t instanceof Advantage))
                            t = Advantage.fromObject(t, this.actor);
                ...
                if (!!t.itemid) {
                    ...
                    if (!!i?.system.globalid) {
                        if (!(t instanceof Skill))
                            t = Skill.fromObject(t, this.actor);
                        t = await this._processItemFrom(t, 'GCA');
                        temp.push(t);
                    }
                }
                ...
                if (!!t.itemid) {
                    ...
                    if (!!i?.system.globalid) {
                        if (!(t instanceof Spell))
                            t = Spell.fromObject(t, this.actor);
                        t = await this._processItemFrom(t, 'GCA');
                        temp.push(t);
                    }
                }
    ...
    async _preImport(generator, itemType) {
        if (!this.isUsingFoundryItems()) {
            ...
            const isEligibleItem = item => {
                const sysKey = itemType === 'equipment'
                    ? this.actor._findEqtkeyForId('itemid', item.id)
                    : this.actor._findSysKeyForId('itemid', item.id, item.actorComponentKey);
                return ((!!item.system.importid && item.system.importFrom === generator && item.type === itemType) ||
            ...
            if (eligibleItems.length > 0)
                await this.actor.deleteEmbeddedDocuments('Item', eligibleItems.map(i => i.id));
        }
    }
    /**
     * @param {{ [key: string]: any }} json
    ...
    async importEquipmentFromGCA(json) {
        ...
        for (let key in json) {
            if (key.startsWith('id-')) {
                ...
                if (!old)
                    old = this._findElementIn('equipment.other', eqt.uuid);
        ...
        await aRecurselist(this.actor.system.equipment?.carried, async (t) => {
            ...
            if (!!t.save) {
                if (!(t instanceof Equipment))
                    t = Equipment.fromObject(t, this.actor);
                t = await this._processItemFrom(t, 'GCA');
                temp.push(t);
            }
        ...
        for (const eqt of temp) {
            ...
            if (!eqt.parentuuid) {
                if (eqt.carried)
                    GURPS.put(equipment.carried, eqt, cindex++);
                else
                    GURPS.put(equipment.other, eqt, oindex++);
            }
        }
        return {
    ...
    async importProtectionFromGCA(json) {
        ...
        let t = this.textFrom;
        let data = this.actor.system;
        if (!!data.additionalresources.ignoreinputbodyplan)
            return;
        ...
        if (!!game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_OVERWRITE_PORTRAITS)) {
            if (p.portrait) {
                if (game.user.hasPermission('FILES_UPLOAD')) {
                    ...
                    await ui.notifications.error('You do not have "FILES_UPLOAD" permission, portrait upload has failed. Please ask your GM to import your character, or acquire the correct permissions.');
                }
            }
        }
        return r;
    ...
    async importAdsFromGCS(ads) {
        let temp = [];
        if (!!ads)
            await this._preImport('GCS', 'feature');
        for (let i of ads) {
    ...
    async importAd(i, p) {
        ...
        if (i.children?.length) {
            for (let j of i.children)
                ch = ch.concat(await this.importAd(j, i.id));
        }
        return [a].concat(ch);
    }
    ...
    async importSk(i, p) {
        ...
        let name = i.name + (!!i.tech_level ? `/TL${i.tech_level}` : '') + (!!i.specialization ? ` (${i.specialization})` : '') ||
            'Skill';
        ...
        if (i.children?.length) {
            for (let j of i.children)
                ch = ch.concat(await this.importSk(j, i.id));
        }
        return [s].concat(ch);
    }
    ...
    async importSp(i, p) {
        ...
        if (i.children?.length) {
            for (let j of i.children)
                ch = ch.concat(await this.importSp(j, i.id));
        }
        return [s].concat(ch);
    }
    ...
    async importEq(i, p, carried) {
        ...
        if (!old)
            old = this._findElementIn('equipment.other', e.uuid);
        ...
        if (i.children?.length) {
            for (let j of i.children)
                ch = ch.concat(await this.importEq(j, i.id, carried));
            for (let j of ch) {
    ...
    importNote(i, p) {
        ...
        if (i.children?.length) {
            for (let j of i.children)
                ch = ch.concat(this.importNote(j, i.id));
        }
        return [n].concat(ch);
    }
    ...
    importPointTotalsFromGCS(total, atts, ads, skills, spells) {
        ...
        let p_race = 0;
        for (let i of atts)
            p_atts += i.calc?.points;
        for (let i of ads)
            [p_ads, p_disads, p_quirks, p_race] = this.adPointCount(i, p_ads, p_disads, p_quirks, p_race, true);
        for (let i of skills)
            p_skills = this.skPointCount(i, p_skills);
        for (let i of spells)
            p_spells = this.skPointCount(i, p_spells);
    ...
    importReactionsFromGCS(ads, skills, equipment) {
        ...
        for (let i of temp_r) {
            ...
            if (!!existing_condition)
                existing_condition.modifier += i.modifier;
            else
                temp_r2.push(i);
        }
        for (let i of temp_c) {
    ...
    calcTotalCarried(eqp) {
        ...
        for (let i of eqp) {
            ...
            if (i.children?.length)
                w += this.calcTotalCarried(i.children);
            t += w;
        }
        return t;
    }
    recursiveGet(i) {
        ...
        if (i.children?.length)
            for (let j of i.children)
                ch = ch.concat(this.recursiveGet(j));
        if (i.modifiers?.length)
            for (let j of i.modifiers)
                ch = ch.concat(this.recursiveGet(j));
        if (!!i.disabled || (i.equipped != null && i.equipped == false))
            return [];
        return [i].concat(ch);
    }
    signedNum(x) {
    ...
    adPointCount(i, ads, disads, quirks, race, toplevel = false) {
        ...
        if (i.type == 'trait_container' && i.container_type == 'race')
            race += i.calc?.points;
        else if (i.type == 'trait_container' && i.container_type == 'alternative_abilities')
            ads += i.calc?.points;
        ...
        else if (i.calc?.points == -1)
            quirks += i.calc?.points;
        else if (i.calc?.points > 0)
            ads += i.calc?.points;
        else
            disads += i.calc?.points;
        return [ads, disads, quirks, race];
    }
    skPointCount(i, skills) {
        if (this.GCSVersion === 5) {
            if (i.id.startsWith('q'))
                i.type = 'technique';
            if (i.id.startsWith('s'))
                i.type = 'skill';
            if (i.id.startsWith('S'))
                i.type = 'skill_container';
            if (i.id.startsWith('r'))
                i.type = 'ritual_magic_spell';
            if (i.id.startsWith('p'))
                i.type = 'spell';
            if (i.id.startsWith('P'))
                i.type = 'spell_container';
        }
    ...
    _migrateOtfsAndNotes(oldobj = {}, newobj, importvttnotes = '', usagenotes = '') {
        ...
        if (!!usagenotes)
            newobj.notes += (!!newobj.notes ? ' ' : '') + usagenotes;
        ...
        if (!newobj.itemModifiers)
            newobj.itemModifiers = oldobj.itemModifiers || '';
        if (!newobj.addToQuickRoll)
            newobj.addToQuickRoll = oldobj.addToQuickRoll || false;
        if (!newobj.modifierTags)
            newobj.modifierTags = oldobj.modifierTags || '';
    }
    /**
    ...
    foldList(flat, target = {}) {
        flat.forEach(obj => {
            if (!!obj.parentuuid) {
                ...
                if (!!parent) {
                    if (!parent.contains)
                        parent.contains = {}; // lazy init for older characters
    ...
    async _processItemFrom(actorComp, fromProgram) {
        if (!!game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_USE_FOUNDRY_ITEMS)) {
            // Sanity check
            if (!(actorComp instanceof Equipment) &&
                ...
                throw new Error('Invalid Actor Component. To process a Item it must be an Equipment, Skill, Spell, Ranged or Melee Attack or Advantage');
            }
