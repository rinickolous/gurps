...
import { arrayToObject, generateUniqueId, makeRegexPatternFrom, objectToArray, recurselist, splitArgs, zeroFill, } from '../../lib/utilities.js';
...
import Maneuvers, { MOVE_HALF, MOVE_NONE, MOVE_ONE, MOVE_ONETHIRD, MOVE_STEP, MOVE_TWO_STEPS, MOVE_TWOTHIRDS, PROPERTY_MOVEOVERRIDE_MANEUVER, PROPERTY_MOVEOVERRIDE_POSTURE, } from './maneuver.js';
...
export class GurpsActor extends Actor {
    ...
    async postImport() {
        ...
        // @ts-ignore
        let orig = /** @type {GurpsItem[]} */ (this.items.contents
        ...
        for (const item of good)
            await this.addItemData(item); // re-add the item equipment and features
            ...
            recurselist(this.system.languages, (e, _k, _d) => {
                ...
                if (a) {
                    ...
                    if (e.spoken == e.written)
                        ...
                        n += ' (' + game.i18n.localize('GURPS.written') + ') (' + e.written + ')';
                    let a = new Advantage();
                    a.name = n;
                    a.points = e.points;
                    GURPS.put(newads, a);
    ...
    calculateDerivedValues() {
        ...
        if (!this.system.equippedparry)
            this.system.equippedparry = this.getEquippedParry();
        if (!this.system.equippedblock)
            this.system.equippedblock = this.getEquippedBlock();
        // Catch for older actors that may not have these values set.
        if (!this.system.currentmove)
            this.system.currentmove = parseInt(this.system.basicmove.value.toString());
        if (!this.system.currentdodge && this.system.dodge.value)
            this.system.currentdodge = parseInt(this.system.dodge.value.toString());
        if (!this.system.currentflight)
            this.system.currentflight = parseFloat(this.system.basicspeed.value.toString()) * 2;
        // Look for Defense bonuses.
        if (!this.system.defenses)
            this.system.defenses = this.getEquippedDefenseBonuses();
        this.ignoreRender = saved;
        if (!saved)
            setTimeout(() => this._forceRender(), 333);
    }
    ...
    _initializeStartingValues() {
        ...
        if (!!data.equipment && !data.equipment.other)
    ...
    _applyItemBonuses() {
        ...
        for (const item of this.items.contents) {
            ...
            if ((item.type !== 'equipment' || (itemData.equipped && itemData.carried)) &&
                !!itemData.bonuses &&
                ...
                for (let bonus of bonuses) {
                    ...
                    if (link.action) {
                        ...
                        if (link.action.type == 'attribute') {
                            ...
                            if (paths.length > 0)
                                data = foundry.utils.getProperty(data, paths.join('.'));
                            // regular attributes have a path
                            else {
                                // only accept DODGE
                                if (link.action.attrkey != 'DODGE')
                                    break;
                            }
    ...
    _findEqtkeyForId(key, id) {
        ...
        recurselist(data.equipment.carried, (e, k, _d) => {
            if (e[key] == id)
                eqtkey = 'system.equipment.carried.' + k;
        });
        if (!eqtkey)
            recurselist(data.equipment.other, (e, k, _d) => {
                if (e[key] == id)
                    eqtkey = 'system.equipment.other.' + k;
            });
        return eqtkey;
    }
    /**
    ...
    _calculateEncumbranceIssues() {
        ...
        if (!!encs) {
            ...
            for (let enckey in encs) {
                let enc = encs[enckey];
                if (!enc.level)
                    enc.level = parseInt(enckey); // FIXME: Why enc.level=NaN after GCA import?
    ...
    _getCurrentMoveMode() {
        let move = this.system.move;
        let current = Object.values(move).find(it => it.default);
        if (!current && Object.keys(move).length > 0)
            return move['00000'];
        return current;
    }
    /**
                        ...
                        GURPS.performAction(action.action, this).then(ret => {
                            ...
                            if (isMelee) {
                                if (!isNaN(parseInt(e.parry))) {
                                    ...
                                    // @ts-ignore
                                    if (!m && p.trim() == '0')
                                        m = [0, 0]; // allow '0' to mean 'no bonus', not skill level = 0
                                    if (!!m) {
    ...
    async replaceManeuver(maneuverText) {
        ...
        if (tokens && tokens.length)
            for (const t of tokens)
                await t.setManeuver(maneuverText);
    }
    ...
    isEffectActive(effect) {
        for (const it of this.effects) {
            if (it.statuses.find(s => s === effect.id))
                return true;
    ...
    async accumulateDamageRoll(action) {
        ...
        if (existing !== -1)
            return this.incrementDamageAccumulator(existing);
    ...
    async setResourceTrackers() {
        ...
        for (const template of templates) {
            ...
            if (!!tracker && !!tracker.name) {
                ui.notifications?.warn(`Will not overwrite Tracker ${template.slot} as its name is set to ${tracker.name}. Create Tracker for ${template.tracker.name} failed.`);
                return;
            }
    ...
    async removeTracker(path) {
        ...
        if (data)
            this.update({ 'system.additionalresources.tracker': data });
        else
            this.update('system.additionalresources.tracker', {});
        this._forceRender();
    }
    async addTracker() {
        this.ignoreRender = true;
    ...
    async handleItemDrop(dragData) {
        ...
        if (!data.globalid)
            await data.update({ _id: data._id, 'system.globalid': dragData.uuid });
        this.ignoreRender = true;
        if (!game.settings.get(settings.SYSTEM_NAME, settings.SETTING_USE_FOUNDRY_ITEMS)) {
            ...
            ui.notifications?.info(game.i18n.format('GURPS.droppingItemNotification', { actorName: this.name, itemName: data.name }));
            // 2. Check if Actor Component exists
            const actorCompKey = data.type === 'equipment'
            ...
            if (!!actorComp) {
                ...
                for (const key of keys) {
                    recurselist(data.system[key], e => {
                        if (!e.uuid)
                            e.uuid = foundry.utils.randomID(16);
                    });
                }
                ...
                if (data.type === 'equipment')
                    await Equipment.calc(actorComp);
    ...
    async handleEquipmentDrop(dragData) {
        if (dragData.actorid == this.id)
            return false; // same sheet drag and drop handled elsewhere
        if (!dragData.itemid) {
        ...
        if ((!!eqt.contains && Object.keys(eqt.contains).length > 0) ||
    ...
    async updateItem(item) {
        ...
        var oldeqt;
        if (!!oldkey)
            oldeqt = foundry.utils.getProperty(this, oldkey);
    ...
    async _addNewItemEquipment(itemData, targetkey) {
        ...
        if (!!_data.eqt.parentuuid) {
            ...
            recurselist(this.system.equipment.carried, (e, k, _d) => {
                if (e.uuid == _data.eqt.parentuuid)
                    found = 'system.equipment.carried.' + k;
            });
            if (!found)
                recurselist(this.system.equipment.other, (e, k, _d) => {
                    if (e.uuid == _data.eqt.parentuuid)
                        found = 'system.equipment.other.' + k;
                });
            if (!!found) {
        ...
        if (targetkey == null)
            if (_data.carried) {
                ...
                while (list.hasOwnProperty(zeroFill(index)))
                    index++;
                targetkey += '.' + zeroFill(index);
            }
            else
                targetkey = 'system.equipment.other';
    ...
    async deleteEquipment(path, depth = 0) {
        ...
        if (!!eqt.itemid) {
            ...
            if (!!item)
                await item.delete(); // data protect for messed up mooks
            await this._removeItemAdditions(eqt.itemid);
        }
    ...
    async moveEquipment(srckey, targetkey, shiftkey) {
        if (srckey == targetkey)
            return;
        if (shiftkey && (await this._splitEquipment(srckey, targetkey)))
            return;
        ...
        for (let i = 0; i < max; i++) {
            if (parseInt(srca[i]) < parseInt(tara[i]))
                isSrcFirst = true;
        }
        ...
        if (await this._checkForMerging(srckey, targetkey))
            return;
    ...
    async _splitEquipment(srckey, targetkey) {
        ...
        let count = 0;
        let callback = async (/** @type {JQuery<HTMLElement>} */ html) => (count = parseInt(html.find('#qty').val()?.toString() || '0'));
        await Dialog.prompt({
            title: 'Split stack',
            label: game.i18n.localize('GURPS.ok'),
            content: content,
        ...
        if (count <= 0)
            return true; // didn't want to split
        if (count >= srceqt.count)
            return false; // not a split, but a move
    ...
    async _checkForMerging(srckey, targetkey) {
        ...
        if ((!!srceqt.globalid && srceqt.globalid == desteqt.globalid) ||
    ...
    get hitLocationsWithDR() {
        ...
        for (const [_key, value] of Object.entries(this.system.hitlocations)) {
            ...
            if (!rollText)
                rollText = HitLocations.HitLocation.DEFAULT;
    ...
    async updateEqtCount(eqtkey, count) {
        ...
        if (!(await this._sanityCheckItemSettings(eqt)))
            return;
        ...
        if (!game.settings.get(settings.SYSTEM_NAME, settings.SETTING_USE_FOUNDRY_ITEMS)) {
            ...
            if (!!eqt.itemid) {
                ...
                if (!!item)
                    await this.updateEmbeddedDocuments('Item', [{ _id: item.id, 'system.eqt.count': count }]);
                else {
    ...
    async _updateEquipmentCalc(equipKey) {
        if (!equipKey.includes('system.eqt.'))
            return;
    ...
    async _updateItemFromForm(item) {
        const sysKey = item.type === 'equipment'
        ...
        if (!(await this._sanityCheckItemSettings(actorComp)))
            return;
        if (!!item.editingActor)
            delete item.editingActor;
        await this._removeItemAdditions(item.id);
    ...
    _changeDR(drFormula, hitLocation) {
        ...
        if (!hitLocation.drItem)
            hitLocation.drItem = 0;
        if (typeof hitLocation.import === 'string')
            hitLocation.import = parseInt(hitLocation.import);
        if (drFormula.startsWith('+') || drFormula.startsWith('-')) {
            ...
            if (!hitLocation.drCap)
                hitLocation.drCap = Math.max(hitLocation.import + hitLocation.drItem, 0);
    ...
    findByOriginalName(name, include = false) {
        ...
        if (!item)
            item = this.items.find(i => i.system.name === name);
        if (!!item)
            return item;
        ...
        for (let key of actorSysKeys) {
            ...
            if (!sysKey)
                sysKey = this._findSysKeyForId('name', name, key, include);
            if (sysKey)
                return foundry.utils.getProperty(this, sysKey);
            ...
            if (localizedName && localizedName !== localizedKey) {
                ...
                if (!sysKey)
                    sysKey = this._findSysKeyForId('name', localizedName, key, include);
                if (sysKey)
                    return foundry.utils.getProperty(this, sysKey);
            }
        }
                ...
                for (const key of attacks) {
                    ...
                    recurselist(this.system[key], (attack, _k, _d) => {
                        if (attack.addToQuickRoll) {
                            ...
                            if (!comp)
                                comp = this.findEquipmentByName(attack.name)[0];
                ...
                for (const key of defenses) {
                    ...
                    recurselist(this.system.melee, (defense, _k, _d) => {
                        if (parseInt(defense[key] || 0) > 0 && defense.addToQuickRoll) {
                            ...
                            if (!comp)
                                comp = this.findEquipmentByName(defense.name)[0];
                            const img = this.items.get(comp?.itemid)?.img;
                            const symbol = game.i18n.localize(`GURPS.${key}`);
    ...
    async addTaggedRollModifiers(chatThing, optionalArgs, attack) {
        ...
        if (optionalArgs.obj) {
            ...
            switch (ref) {
                ...
                case 'dodge':
                    refTags = taggedSettings.allDefenseRolls.split(',').map(it => it.trim().toLowerCase());
                    refTags = refTags.concat(taggedSettings[`all${ref.toUpperCase()}Rolls`].split(',').map(it => it.trim().toLowerCase()));
                    break;
                case 'p':
                    refTags = taggedSettings.allDefenseRolls.split(',').map(it => it.trim().toLowerCase());
                    refTags = refTags.concat(taggedSettings.allParryRolls.split(',').map(it => it.trim().toLowerCase()));
        ...
        const userMods = foundry.utils.getProperty(this, 'system.conditions.usermods') || [];
        // Actor Self Mods
        const selfMods = foundry.utils.getProperty(this, 'system.conditions.self.modifiers').map(mod => {
            const key = mod.match(/(GURPS.\w+)/)?.[1] || '';
            if (key)
                return game.i18n.localize(key) + mod.replace(key, '');
            return mod;
        }) || [];
        // Actor Targeted Mods
        let targetMods = [];
        for (const target of Array.from(game.user.targets)) {
            ...
            for (const mod of mods) {
                const key = mod.match(/(GURPS.\w+)/)?.[1] || '';
                if (key)
                    targetMods.push(game.i18n.localize(key) + mod.replace(key, ''));
                else
                    targetMods.push(mod);
            }
    ...
    async canRoll(action, token, chatThing = '', actorComp = {}) {
        ...
        if (token && combatIsActive && isCombatant && !isSlam) {
            ...
            if ((!actions.canAttack && isAttack) || (!actions.canDefend && isDefense)) {
                ...
                const checkManeuverSettings = game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_ALLOW_ROLL_BASED_ON_MANEUVER);
                const message = checkManeuverSettings !== 'Allow' &&
            ...
            if (!isAttack &&
                ...
                result = {
                    ...
                    hasActions: false,
                    maxActionMessage: checkMaxActionSettings !== 'Allow' &&
            ...
            if (isAttack &&
                ...
                result = {
                    ...
                    canRoll: result.canRoll && checkMaxActionSettings !== 'Forbid',
                    hasActions: false,
                    maxAttackMessage: checkMaxActionSettings !== 'Allow' &&
            ...
            if (isDefense &&
                ...
                result = {
                    ...
                    canRoll: result.canRoll && checkMaxActionSettings !== 'Forbid',
                    hasActions: false,
                    maxBlockMessage: checkMaxActionSettings !== 'Allow' &&
            ...
            if (isDefense &&
                ...
                result = {
                    ...
                    canRoll: result.canRoll && checkMaxActionSettings !== 'Forbid',
                    hasActions: false,
                    maxParryMessage: checkMaxActionSettings !== 'Allow' &&
                        game.i18n.localize(`GURPS.${checkMaxActionSettings.toLowerCase()}MaxParriesReached`),
                };
            }
            // Check if Combat is started
            if (!combatStarted) {
                const checkCombatSettings = game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_ALLOW_ROLLS_BEFORE_COMBAT_START);
                result = {
                    ...result,
                    canRoll: result.canRoll && checkCombatSettings !== 'Forbid',
                    rollBeforeStartMessage: checkCombatSettings !== 'Allow' &&
        ...
        if (isCombatant && needTarget && game.user.targets.size === 0) {
            result = {
                ...
                targetMessage: checkForTargetSettings !== 'Allow' &&
    ...
    findUsingAction(action, chatting, formula, thing) {
        ...
        switch (originType) {
            ...
            case 'attribute':
                ...
                if (!attrName)
                    attrName = game.i18n.localize(`GURPS.${action.attrkey.toLowerCase()}`);
                if (attrName.startsWith('GURPS'))
                    attrName = game.i18n.localize(`GURPS.attributes${action.attrkey}NAME`);
                result = {
                    name: attrName,
                    uuid: null,
                    itemId: null,
