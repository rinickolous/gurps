        ...
        if (actor) {
            ...
            if (taggedSettings.checkConditionals) {
                ...
                recurselist(conditionalMods, (e, _k, _d) => {
                    ...
                    if (mod !== 0)
                        sheetMods.push(`${signal}${Math.abs(mod)} ${e.situation} @${source}`);
                });
            }
    ...
    addUserMod(event) {
        if (this.getToken()) {
            ...
            Dialog.prompt({
                ...
                content: "<input type='text' id='GURPS-user-mod-input' style='text-align: left;' placeholder ='Ex.: +1 GM&#39s Luck #hit #damage #check #combat'>'",
 ...
 * @returns {string[]}
 */
export const cleanTags = tags => (tags || '')
    .split(',')
    .map(it => it.trim())
    .filter(it => !!it)
