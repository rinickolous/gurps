...
export class EffectModifierControl {
    ...
    _registerSetting() {
        game.settings.register(SYSTEM_NAME, EffectModifierControl.SETTING_SHOW_EFFECTMODIFIERS, {
            ...
            hint: game.i18n.localize('GURPS.settingHintTokenToolsShowEffectMods', 'Enable the token Effect Modifiers popup window.'),
            scope: 'client',
            config: true,
            type: Boolean,
            default: true,
    ...
    _updatedActiveEffect(effect, _, __) {
        ...
        if (effect?.parent.id === this.token?.actor.id)
    ...
    _updateToken(tokenDocument) {
        ...
        if (tokenDocument.object === this.token)
            this._ui.render(false);
    }
