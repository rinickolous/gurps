...
export class GurpsActorSheet extends ActorSheet {
    ...
    getData() {
        ...
        let actions = {};
        if (!this.actor.system.conditions.actions?.maxActions)
            actions['maxActions'] = 1;
        if (!this.actor.system.conditions.actions?.maxBlocks)
            actions['maxBlocks'] = 1;
        if (Object.keys(actions).length > 0)
            this.actor.internalUpdate({ 'system.conditions.actions': actions });
    ...
    activateListeners(html) {
        ...
        html.find('[data-otf]').each((_, li) => {
            ...
            li.addEventListener('dragstart', ev => {
                ...
                return ev.dataTransfer.setData('text/plain', JSON.stringify({
        ...
        if (!isConfigurationAllowed(this.actor))
            return; // Only allow "owners" to be able to edit the sheet, but anyone can roll from the sheet
        ...
        html.find('button[data-operation="resource-inc"]').click(async (ev) => {
            ...
            if (isNaN(value))
                value = tracker.max || 0;
            if (tracker.isMinimumEnforced && value < tracker.min)
                value = tracker.min;
            if (tracker.isMaximumEnforced && value > tracker.max)
                value = tracker.max;
        ...
        html.find('button[data-operation="ci-days-inc"]').click(async (ev) => {
            ev.preventDefault();
            updateActorWithChangedDaysToHeal(CI.incrementDaysToHeal(this.actor.system.conditionalinjury.injury.daystoheal, ev.shiftKey ? 5 : 1));
        });
        html.find('button[data-operation="ci-days-dec"]').click(ev => {
            ev.preventDefault();
            updateActorWithChangedDaysToHeal(CI.decrementDaysToHeal(this.actor.system.conditionalinjury.injury.daystoheal, ev.shiftKey ? 5 : 1));
        });
        ...
        html.find('.dblclkedit').dblclick(async (ev) => {
            ...
            if (!!obj.itemid) {
                if (!(await this.actor._sanityCheckItemSettings(obj)))
                    return;
            ...
            if (path.includes('equipment'))
                await this.editEquipment(actor, path, obj);
            if (path.includes('melee'))
                await this.editMelee(actor, path, obj);
            if (path.includes('ranged'))
                await this.editRanged(actor, path, obj);
            if (path.includes('ads'))
                await this.editAds(actor, path, obj);
            if (path.includes('skills'))
                await this.editSkills(actor, path, obj);
            if (path.includes('spells'))
                await this.editSpells(actor, path, obj);
            if (path.includes('notes'))
                await this.editNotes(actor, path, obj);
        });
        ...
        html.find('i.equipmentbutton[data-operation="equipment-inc-uses"]').click(async (ev) => {
            ...
            let path = parent.attr('data-key');
            let eqt = foundry.utils.getProperty(this.actor, path);
            if (!(await this.actor._sanityCheckItemSettings(eqt)))
                return;
        ...
        html.find('[data-onethird]').hover(function () {
            let opt = $(this).attr('data-onethird');
            let msg = 'Disable&nbsp;' + opt;
            if ($(this).hasClass('buttongrey'))
                msg = 'Enable&nbsp;' + opt;
            $(this).append($(`<div style='font-family: Roboto, sans-serif; position: absolute;z-index: 10;top: 10px;left: 100%;padding: 5px;width=120px;color:#9f0000;background-color:lightgrey;border: 1px solid grey;border-radius:5px'>${msg}</div>`));
        }, function () {
    ...
    _createHeaderMenus(html) {
        ...
        if (!!trackermenu.length) {
            this._makeHeaderMenu($(trackermenu[0]), '.headermenu', [
                {
    ...
    _createGlobalItemMenus(html) {
        let opts = [
            this._createMenu(game.i18n.localize('GURPS.delete'), '<i class="fas fa-trash"></i>', this._deleteItem.bind(this), this._isRemovable.bind(this)),
        ];
    ...
    _createEquipmentItemMenus(html) {
        ...
        let opts = [
            ...
            this._createMenu(game.i18n.localize('GURPS.sortContentsAscending'), '<i class="fas fa-sort-amount-down-alt"></i>', this._sortContentAscending.bind(this), this._isSortable.bind(this, includeCollapsed)),
            this._createMenu(game.i18n.localize('GURPS.sortContentsDescending'), '<i class="fas fa-sort-amount-down"></i>', this._sortContentDescending.bind(this), this._isSortable.bind(this, includeCollapsed)),
            this._createMenu(game.i18n.localize('GURPS.delete'), '<i class="fas fa-trash"></i>', this._deleteItem.bind(this)),
        ];
        let movedown = this._createMenu(game.i18n.localize('GURPS.moveToOtherEquipment'), '<i class="fas fa-level-down-alt"></i>', this._moveEquipment.bind(this, 'system.equipment.other'));
        new ContextMenu(html, '.equipmenucarried', [movedown, ...opts], { eventName: 'contextmenu' });
        let moveup = this._createMenu(game.i18n.localize('GURPS.moveToCarriedEquipment'), '<i class="fas fa-level-up-alt"></i>', this._moveEquipment.bind(this, 'system.equipment.carried'));
    ...
    _isSortable(includeCollapsed, target) {
        ...
        if (x?.contains && Object.keys(x.contains).length > 1)
            return true;
        if (includeCollapsed)
    ...
    getMenuItems(elementid) {
        const map = {
            ...
            '#equipmentcarried': [
                this.addItemMenu(game.i18n.localize('GURPS.equipment'), new Equipment(`${game.i18n.localize('GURPS.equipment')}...`, true), 'system.equipment.carried'),
                this.sortAscendingMenu('system.equipment.carried'),
                this.sortDescendingMenu('system.equipment.carried'),
            ],
            '#equipmentother': [
                this.addItemMenu(game.i18n.localize('GURPS.equipment'), new Equipment(`${game.i18n.localize('GURPS.equipment')}...`, true), 'system.equipment.other'),
        ...
        return {
            ...
            callback: async (e) => {
                if (path.includes('system.equipment')) {
                    ...
                    if (!obj.uuid)
                        obj.uuid = obj._getGGAId({ name: obj.name, type: path.split('.')[1], generator: '' });
                }
    ...
    makelistdrag(html, cls, type) {
        html.find(cls).each((i, li) => {
            ...
            li.addEventListener('dragstart', ev => {
                ...
                if (!!oldd)
                    foundry.utils.mergeObject(newd, JSON.parse(oldd)); // May need to merge in OTF drag info
    ...
    dropFoundryLinks(ev, modelkey) {
        ...
        if (!!dragData.uuid)
            dragData.id = dragData.uuid.split('.').at(1);
        let add = '';
        ...
        if (!!dragData.otf) {
            ...
            if (!!dragData.displayname) {
                let q = '"';
                if (dragData.displayname.includes(q))
                    q = "'";
                ...
                one: {
                    ...
                    callback: async (html) => {
                        if (!game.settings.get(Settings.SYSTEM_NAME, Settings.SETTING_USE_FOUNDRY_ITEMS)) {
                            ...
                            let u = html.find('.save'); // Should only find in Note (or equipment)
                            if (!!u && obj.save != null)
                                obj.save = u.is(':checked'); // only set 'saved' if it was already defined
                            let v = html.find('.ignoreImportQty'); // Should only find in equipment
                            if (!!v)
                                obj.ignoreImportQty = v.is(':checked');
    ...
    async editMelee(actor, path, obj) {
        if (obj.baseParryPenalty === undefined)
            obj.baseParryPenalty = -4;
        if (obj.extraAttacks === undefined)
            obj.extraAttacks = 0;
        if (obj.consumeAction === undefined)
            obj.consumeAction = true;
        await this.editItem(actor, path, obj, 'systems/gurps/templates/melee-editor-popup.hbs', 'Melee Weapon Editor', [
            'name',
            'mode',
            'parry',
            'block',
    ...
    async editRanged(actor, path, obj) {
        ...
        if (obj.consumeAction === undefined)
            obj.consumeAction = true;
        await this.editItem(actor, path, obj, 'systems/gurps/templates/ranged-editor-popup.hbs', 'Ranged Weapon Editor', [
            'name',
            'mode',
            'range',
            'rof',
    ...
    async editAds(actor, path, obj) {
        this.editItem(actor, path, obj, 'systems/gurps/templates/advantage-editor-popup.hbs', 'Advantage / Disadvantage / Perk / Quirk Editor', ['name', 'notes', 'pageref', 'itemModifiers'], ['points']);
    }
    async editSkills(actor, path, obj) {
        if (obj.consumeAction === undefined)
            obj.consumeAction = false;
        await this.editItem(actor, path, obj, 'systems/gurps/templates/skill-editor-popup.hbs', 'Skill Editor', [
            'name',
            'import',
            'relativelevel',
            'pageref',
    ...
    async editSpells(actor, path, obj) {
        if (obj.consumeAction === undefined)
            obj.consumeAction = true;
        await this.editItem(actor, path, obj, 'systems/gurps/templates/spell-editor-popup.hbs', 'Spell Editor', [
            'name',
            'import',
            'difficulty',
            'pageref',
    ...
    async editNotes(actor, path, obj) {
        this.editItem(actor, path, obj, 'systems/gurps/templates/note-editor-popup.hbs', 'Note Editor', ['pageref', 'notes', 'title'], [], 730);
    }
            ...
            buttons: {
                one: {
                    ...
                    callback: async (html) => {
                        ...
                        if (!!q)
                            obj.addToQuickRoll = q.is(':checked');
                        let ca = html.find('.consumeAction');
                        if (!!ca)
                            obj.consumeAction = ca.is(':checked');
                        let u = html.find('.save'); // Should only find in Note (or equipment)
                        if (!!u)
                            obj.save = u.is(':checked');
                        if (!!obj.modifierTags)
    ...
    async _onDrop(event) {
        ...
        if (dragData.type === 'Item')
            await this.actor.handleItemDrop(dragData);
        ...
        if (dragData.type === 'equipment') {
            if ((await this.actor.handleEquipmentDrop(dragData)) !== false)
                return; // handle external drag/drop
            ...
            if (dropTargetElements?.length === 0)
                return;
    ...
    async _onToggleSheet(event, altsheet) {
        ...
        if (original != 'gurps.GurpsActorSheet')
            newSheet = 'gurps.GurpsActorSheet';
        if (event.shiftKey)
    ...
    async _onNavigate(event) {
        ...
        if (!target)
            return; // If they click on a section that isn't on the sheet (like ranged)
    ...
    async _onClickEquip(ev) {
        ...
        let element = ev.currentTarget;
        let key = element.dataset.key;
        if (!(await this.actor._sanityCheckItemSettings(GURPS.decode(this.actor, key))))
            return;
...
export class GurpsActorEditorSheet extends GurpsActorSheet {
    ...
    getMenuItems(elementid) {
        ...
        switch (elementid) {
            ...
            case '#reactions':
                return [
                    this.addItemMenu(game.i18n.localize('GURPS.reaction'), new Reaction('+0', game.i18n.localize('GURPS.fromEllipses')), 'system.reactions'),
                    ...menu,
                ];
            case '#conditionalmods':
                return [
                    this.addItemMenu(game.i18n.localize('GURPS.conditionalModifier'), new Modifier('+0', game.i18n.localize('GURPS.fromEllipses')), 'system.conditionalmods'),
                    ...menu,
                ];
            case '#melee':
                return [
                    this.addItemMenu(game.i18n.localize('GURPS.meleeAttack'), new Ranged(`${game.i18n.localize('GURPS.meleeAttack')}...`), 'system.melee'),
                    ...menu,
                ];
            case '#ranged':
                return [
                    this.addItemMenu(game.i18n.localize('GURPS.rangedAttack'), new Ranged(`${game.i18n.localize('GURPS.rangedAttack')}...`), 'system.ranged'),
                    ...menu,
                ];
            case '#advantages':
                return [
                    this.addItemMenu(game.i18n.localize('GURPS.adDisadQuirkPerk'), new Advantage(`${game.i18n.localize('GURPS.adDisad')}...`), 'system.ads'),
                    ...menu,
                ];
            case '#skills':
                return [
                    this.addItemMenu(game.i18n.localize('GURPS.skill'), new Skill(`${game.i18n.localize('GURPS.skill')}...`), 'system.skills'),
                    ...menu,
                ];
            case '#spells':
                return [
                    this.addItemMenu(game.i18n.localize('GURPS.spell'), new Spell(`${game.i18n.localize('GURPS.spell')}...`), 'system.spells'),
                    ...menu,
                ];
            default:
                return menu;
