...
export default class GurpsWiring {
    ...
    static handleGurpslink(event, actor, desc, options) {
        ...
        if (!action && element.dataset?.otf)
            action = parselink(element.dataset.otf, desc).action;
        if (options?.combined) {
    ...
    static onRightClickGurpslink(event) {
        ...
        if (!!action) {
            ...
            if (action.type === 'damage' || action.type === 'deriveddamage')
                ...
                GURPS.whisperOtfToOwner(action.orig, action.overridetxt, event, action, GURPS.LastActor); // only offer blind rolls for things that can be blind, No need to offer blind roll if it is already blind
        }
    }
